...
public class DecryptingFeignClient extends Default {
	...
	public DecryptingFeignClient(SSLSocketFactory sslContextFactory, HostnameVerifier hostnameVerifier,
			String aesSecret) {
	...
	private String decryptDataAES(String encryptedDataInBase64, byte[] key) {
		...
		try {
			...
			byte[] plainText = cipher.doFinal(Arrays.copyOfRange(encryptedDataBytes, 12, encryptedDataBytes.length));
