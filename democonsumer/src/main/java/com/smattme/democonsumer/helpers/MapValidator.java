package com.smattme.democonsumer.helpers;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.jayway.jsonpath.Configuration;
import com.jayway.jsonpath.JsonPath;
import com.smattme.democonsumer.config.SpringContext;
...
public class MapValidator {
	...
	public static List<String> validate(String jsonRequest, Map<String, String> rules) {
		...
		keys.forEach(key -> {
			...
			if (StringUtils.isEmpty(keyRule)) //this key doesn't have a rule
				return;
	...
	public static List<String> validate(Object target, Map<String, String> rules) {
		...
		String jsonRequest = Try.of(() -> objectMapper.writeValueAsString(target))
				.getOrElseThrow(throwable -> new CustomApplicationException(HttpStatus.BAD_REQUEST, "Error parsing request body as JSON"));
	...
	public static boolean validateDigit(Object value) {
		return value != null && !((String) value).isEmpty() && ((String) value).matches("[0-9]+");
	}
	...
	private static Object getValueForField(Object document, String key) {
		return Try.of(() -> JsonPath.read(document, "$." + key))
				.onFailure(throwable -> logger.debug("Error while extracting value for " + key + " : " + throwable.getMessage()))
				.getOrNull();
	}
